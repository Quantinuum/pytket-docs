...
To test the circuit, compile and process the circuit using a {py:class}`~pytket.backends.Backend` that supports mid-circuit measurement and reset (e.g. {py:class}`~pytket.extensions.qiskit.backends.aer.AerBackend` from `pytket-qiskit`).
